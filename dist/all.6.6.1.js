...
function ContentView(oG) {
	...
	const p = createjs.extend(ContentView, createjs.Container);

	const stringDefault = 'Choose a set of characters';

	const setTitles = ['HSK 1+2', 'HSK 3', 'HSK 4a', 'HSK 4b', 'HSK 5a', 'HSK 5b', 'HSK 5c', 'HSK 5d', 'HSK 5e'];
...
function ContentItem(oG) { // checked
	...
	const p = createjs.extend(ContentItem, createjs.Container);
...
(function (oG) { // checked
	...
	function initLoc() {
		...
		if (window.location.host === oG.model.siteUrl) {
			oG.view.init();
		} else if (oG.model.siteLock === false) {
			oG.view.init();
		}
	}
...
(function (oG) { // checked
	...
	const p = createjs.extend(CountdownTimer, createjs.Container);

	p.setup = function () {
		this.countdownVar = 0;
		this.countdownAddVar = 0;

		const countdownBack = new createjs.Sprite(oG.model.mainSprite);
		countdownBack.gotoAndStop('countdownBack');
		const countdownFront = new createjs.Sprite(oG.model.mainSprite);
	...
	p.countdown = function (e) {
		...
		if (!this.paused) {
			...
			const adjVar = Math.round(e.delta) * 25;
			const adjVar2 = Math.floor(this.speedAlt * adjVar) / 1000;
			if (oG.model.optShowTimer) this.countdownVar -= adjVar2;
			if (this.countdownVar <= 0) {
				this.countdownVar = 0;
				oG.view.gameView.timeout();
			}
			const bWidth = Math.round(this.countdownVar / 10);
			if (bWidth > 4) {
	...
	p.deit = function () {
		const gTime = Math.floor(this.gameCount / 30);
...
(function (oG) { // checked
	...
	const p = createjs.extend(EndView, createjs.Container);
	...
	p.gotScoresFun = function (jsonArr) {
		const len = jsonArr.scores.length;
		this.showInput = true;
		if (len > 25) {
			const minScore = jsonArr.scores[len - 1].Score;
			const minTime = jsonArr.scores[len - 1].Tome;
...
(function (oG) { // checked
	...
	const p = createjs.extend(GameView, createjs.Container);
	...
	p.turnOver = function () {
		if (!oG.model.sdStart) {
			const leArr = oG.metrics.getRound();
			if (leArr[0] != -1) {
				this.routine(leArr);
			} else {
				this.winSeq();
	...
	p.routine = function (leArr) {
		...
		const tVar = Math.round(600 * oG.model.gameSpeed);
	...
	p.correctHit = function () {
		...
		const tVar = Math.round(1000 * oG.model.gameSpeed);
	...
	p.updateDispFromSettings = function () {
		...
		} else if (oG.model.orientation === 0) {
			this.audioBut.x = 580;
		} else {
			this.audioBut.x = 450;
		}
...
(function (oG) { // checked
	...
	const p = createjs.extend(HighScoresTable, createjs.Container);
	...
	p.stopUp = function (event) {
		...
		if (this.delta === 0) {
			for (let i = 0; i < this.rows; i++) { this.scrollUp(); }
		}
		this.delta = 0;
	};
	...
	p.stopDown = function (event) {
		...
		if (this.delta === 0) {
			for (let i = 0; i < this.rows; i++) { this.scrollDown(); }
		}
		this.delta = 0;
	};
	...
	p.addScoresFunMovesTime = function () {
		...
		for (i = 0; i < this.rowsCount; i++) {
			...
			const myMon = monthText[myDate.getMonth()];
...
(function (oG) { // checked
	...
	const p = createjs.extend(OptionsPane, createjs.Container);

	const countActiveText = 'Countdown is active';
	const countDeactiveText = 'Countdown is not active';
...
(function (oG) {
    ...
    function setupSpriteSheet(event) {
        const mainSheet = new createjs.SpriteSheet({
...
(function (oG) { // checked
	...
	const p = createjs.extend(PreloadView, createjs.Container);
	...
	p.loadLine = function (event) {
		...
		const bitVar = Math.floor(this.loadLineVar / 2);
		for (let i = 0; i < 8; i++) {
			if (bitVar < i) {
				this.preloadBits[i].visible = false;
			} else {
...
(function (oG) { // checked
	...
	const p = createjs.extend(ReviewView, createjs.Container);
...
(function (oG) { // checked
	...
	const p = createjs.extend(ScorePane, createjs.Container);
	const paneBord = 8;
	...
	p.setupDisplay = function () {
		...
		let entryPaneHeight = 160;
		let entryPaneWidth = 120;
		let scoreDispPaneHeight = 0;
		let scoreDispPaneWidth = 120;
		if (oG.model.orientation === 0) {
		...
		const wid = entryPaneWidth - paneBord;
		const hei = entryPaneHeight - paneBord;
		const sHei = scoreDispPaneHeight - paneBord;
		const sWid = scoreDispPaneWidth - paneBord;
	...
	p.updateInputs = function () {
		if (this.inputActive == true) {
			const rat = oG.model.canvasRatio;
			const wid = Math.round(80 * rat);
			const hei = Math.round(20 * rat);
			const paneX = this.entryPaneContainer.x * rat;
			const paneY = this.entryPaneContainer.y * rat;
			const xOff = Math.round(this.myCan.offsetLeft - wid / 2 + paneX);
			const yOff = Math.round(this.myCan.offsetTop - hei / 2 + paneY);

			let newFontSize = Math.round(20 * rat);
	...
	p.submitClick = function (event) {
		this.cleanInputs();
		const nom = this.nObj.htmlElement.value;
		const loc = this.lObj.htmlElement.value;
...
(function (oG) { // checked
	...
	const p = createjs.extend(SettingsPane, createjs.Container);
...
(function (oG) {
	...
	const p = createjs.extend(SettingsBut, createjs.Container);
	const WID = 140;
	const HEI = 40;
	...
	p.setup = function () {
		...
		if (this.tar == 'char') {
			const charBit = new createjs.Sprite(oG.model.mainSprite);
			...
			const txt = new createjs.Text(str, 'bold 20px Alegreya Sans', '#111');
...
(function (oG) { // checked
	const TargetSet = function () {
		this.Container_constructor();
		this.setup();
	};
	const p = createjs.extend(TargetSet, createjs.Container);
	...
	const charY = [0, 0, 0, -14, -12, -24, 0, 0];
	const charSize = [1, 0, 0, 0.6, 0.6, 0.5, 0, 0];
	const engY = [9, 9, 9, 9, 34, 40, 28, 9];
	const engSize = [0, 30, 0, 0, 20, 20, 24, 0];
	const pinY = [9, 9, 9, 34, 9, 18, -2, 9];
	const pinSize = [0, 0, 32, 24, 0, 24, 32, 0];
	...
	p.showTarget = function (tVar) {
		...
		const oVar = Math.round(600 * oG.model.gameSpeed);
		if (oVar > 0) {
	...
	p.adjDisp = function () {
		const curMode = oG.model.tarView;
		if (curMode == 7) {
			this.visible = false;
		} else {
			this.visible = true;
		}
		const cSize = charSize[curMode];
		const eSize = engSize[curMode];
		const pSize = pinSize[curMode];
		if (cSize === 0) {
			this.cText.visible = false;
		} else {
			this.cText.visible = true;
...
(function (oG) { // checked
	...
	const p = createjs.extend(TitleView, createjs.Container);
	...
	p.setup = function () {
		...
		const dummyText1 = new createjs.Text('.', 'bold 12px Alegreya Sans', '#444');
		const dummyText2 = new createjs.Text('.', 'bold 12px Amaranth', '#444');
		const dummyText3 = new createjs.Text('.', 'bold 12px Ubuntu', '#444');
		const dummyText4 = new createjs.Text('.', 'bold 12px Cabin', '#444');
...
(function (oG) {
	...
	p.setup = function () {
		...
		const txt = new createjs.Sprite(oG.model.mainSprite);
...
(function (oG) { // checked
	...
	const p = createjs.extend(View, createjs.Container);
